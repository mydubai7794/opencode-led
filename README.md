...
- **一键安装**：`bash install.sh` 安装依赖 + 部署插件，重启 OpenCode 即可使用
...
cd opencode-led
bash install.sh        # 安装依赖 + 部署插件
opencode               # 插件自动启动 broker，LED 灯直接可用
```
